 ...
 *                                autoplay is on
 *   arrows="yes|no"              previous / next controls
 *   dots="yes|no"                the position dots
 *
...
function sbvit_shortcode( $atts ) {
	...
	<section class="sbvit" data-sbvit='<?php echo esc_attr( wp_json_encode( $config ) ); ?>'
		...
		<div class="sbvit__card">
			<span class="sbvit__mark" aria-hidden="true">&ldquo;</span>

			<?php
			// The viewport's height is animated to the active slide by
			// the script. Without JS it has no height set at all, so
			// every testimonial simply stacks and stays readable.
			?>
		...
		<?php if ( $total > 1 && ( 'yes' === $atts['arrows'] || 'yes' === $atts['dots'] ) ) : ?>
